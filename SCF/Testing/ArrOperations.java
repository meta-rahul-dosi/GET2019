public class ArrOperations {
	...
	public int findMaxMirror(int[] nums) throws AssertionError {
		int maxLengthMirror = 0;
		int mirrorLength = 0;
									
		if (nums.length == 0) {
			throw new AssertionError(
					"Assertion Error Occured: Please don't pass an empty array");
		}
	...
	public int countClumps(int[] nums) {
		...
		if (nums.length == 0) {
			throw new AssertionError(
					"Assertion Error Occured: an empty array passed");
		}
	...
	/**
	 * returns the split index so that sum of the number on one side is equal to
	 * the sum of the numbers on the other side
	 * 
	 * @param nums
	 *            , array of Integers
	 * @return splitIndex, if found any split position then return split
	 *         position o/w returns -1
	 */
	...
	/**
	 * returns an array that contains exactly the same numbers as the input
	 * array, ,but rearranged so that every X is immediately followed by a Y,
	 * requires that X is fixed at its position we can move Y from its position.
	 * 
	 * @param nums
	 *            , array of integers
	 * @param x
	...
	public int[] fixXY(int[] nums, int x, int y) {
		...
		for (int index = 0; index < nums.length; index++) {
			if (nums[index] == x) {
				...
				if (nums[index] == nums[index + 1]) {
					throw new AssertionError(
							"Assertion Error occured: adjacent X values are there");
				}
			}
			if (nums[index] == y) {
				noOfY++;
		...
		if (noOfX != noOfY) {
			throw new AssertionError(
					"Assertion Error Occured: unequal number of X and Y ");
		}

		// if X occurs at last index of array throw Assertion error
		if (nums[nums.length - 1] == x) {
			throw new AssertionError(
					"Asertion Error occured: x occurs at the last index of array");
		}
