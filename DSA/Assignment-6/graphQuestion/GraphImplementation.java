 ...
 */
public class GraphImplementation implements IGraph {
	...
	public boolean isConnected() {
		return (this.numberOfVertices <= 0) ? false
				: this.dfs(0).size() == this.getNumberOfVertices();
	}
	 ...
	 * 
	 * @param vertex
	 *            whose reachable nodes to be return.
	...
	public List<Edge> minimumSpanningTree() {
		...
		for (Edge edge : this.getEdgeList()) {
			if (index > this.getNumberOfVertices() - 1) {
				break;
			} else if (isCycle(parentVertices, edge.getVertex1(),
					edge.getVertex2()) == false) {
	 ...
	 * @param vertex
	 *            vertex whose parent to be find.
