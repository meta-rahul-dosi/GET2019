...
public class BSTDictionary {
	...
	/**
	 * function used to search element in BST and return the corresponding value
	 * of node
	 * 
	 * @param key
	 * @return
	...
	void insert(String key, String value) {
		...
		if (this.root == null) {
			...
			while (iterator != null) {
				...
				// if key is lesser than iterator key then move to left sub tree
				if (key.compareTo(iterator.key) < 0) {
					iterator = iterator.left;
				}

				// if key is greater then iterator key then move to right sub
				// tree
	...
	/**
	 * we use this function to find the in-order successor of the candidate node
	 * to be deleted
	 * 
	 * @param root
	 *            , the root node of which we find the in-order successor
	...
	/**
	 * function to in-order traverse the BST and return the sorted values b/w
	 * two keys
	 * 
	 * @param node
	 *            , root node
	...
	public static void main(String[] args) {
		...
		System.out.println();
		BSTLinkedList sortedListByKey = dict.getSortedValues(dict.root, "CO",
				"RIP");
