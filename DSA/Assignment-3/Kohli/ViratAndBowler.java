...
public class ViratAndBowler {
	 ...
	 *            , #bowlers
	 * @param viratBowls
	 *            , virat bats no. of bowls
	 * @param bowlArray
	 *            , array containing each bowler bowls
	 */
	public ViratAndBowler(int bowlerNum, int viratBats, int[][] bowlArray) {
		...
		if (this.viratBats > bowlsSum) {
			throw new AssertionError("Virat can't play more balls than bowler");
		}
	}
	...
	public int[][] maxHeapyfy(int[][] bowlArray) {
		...
		for (int heapyfyNum = 0; heapyfyNum <= noOfHeapyfyRequired; heapyfyNum++) {
			for (int count = 0; count < arrLength / 2 - 2; count++) {
				// if left child has greater value than parent then swap the
				// value
				...
				// if the right child has greater value than parent then swap
				// the value
	...
	public static void main(String[] args) {
		int[][] bowlerArray = new int[][] { { 0, 5 }, { 1, 15 }, { 2, 10 },
				{ 3, 8 }, };
