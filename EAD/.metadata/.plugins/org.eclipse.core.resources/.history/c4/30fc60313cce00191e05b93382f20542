...
public class FetchEmployeeData extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		try {
			...
			String query = "SELECT * FROM `EAD-4`.`emp_details` AS ED "
					+ " WHERE ED.email=\"" + email + "\";";
			// PreparedStatement st = dbConn.estabConn(query);

			ResultSet rs = st.executeQuery(query);
			rs.next();
			String p1 = rs.getString(5), p2 = password;
			if (p1.equals(p2)) {
				...
				out.print(".topnav {" + "  overflow: hidden;"
						...
						+ "right: 0px;" + "top: 0px;"
						+ "background-color: white;" + "width: 120;"
						+ "height: 150}"
				...
				out.print("<div class='topnav' name='topnav'>"
						+ "<a  href='#home'>Home</a>"
						+ "<a href='friends?empId=" + rs.getInt(1)
						+ "'>Friends</a>" + "</div>" + "<br/>");
				...
				out.print("<button type=button  onclick='#'>change</button>"
						+ "</div>");
				out.println("<body><h1>Welcome !<br/> " + name
						+ "</h1><br/><br/>");
				out.print("<p>gender: " + gender + "<br/>");
				out.print("<p>contact: " + contact + "<br/>");
				out.print("<p>organization: " + organization + "</p><br/><br/>");

				out.print("<button type=button onclick=\"window.location.href = 'UpdateStudent?id="
						+ rs.getInt(1)
						+ "&fullName="
						+ rs.getString(2)
						+ "&gender="
